...
`devgraph add`/`rescan` now run a real full scan (`devgraph/indexer/dispatch.py`), and the watcher/tray app wire live changes through the same dispatcher, including file-delete cleanup (`GraphEngine.delete_nodes_by_source_file`) — this closes what was previously the biggest gap: extractors existed but nothing ever called them automatically. Also fixed while wiring this up, both confirmed against a real ~1300-node repo (RAG4, in the parent folder — useful for realistic-scale testing): (1) the Python indexer's relative-import (`from . import x`) target naming never matched any real `Module` node, so `IMPORTS` edges silently never materialized for same-repo imports — `find_related_files`'s `imported_modules` was always empty regardless of indexing; (2) `summarise_repository`'s Cypher chained 9 independent `OPTIONAL MATCH` patterns, which Neo4j cross-products before counting — at real-repo scale (hundreds of Function/Class nodes) this hung indefinitely. Fixed via per-count `CALL () {...}` subqueries.

**Known remaining gap**: extractors don't cross-link with each other — a `Service` node from a compose file and a `Database`/`Endpoint` node from Python source have no automatic relationship between them, so `explain_architecture`'s `uses`/`calls` output stays empty even on a fully-indexed repo. Also still true: no `CALLS` edge extraction (`find_callers`/`impact_analysis` only see `CONTAINS`/`IMPORTS`/`EXTENDS`/`USES`), and multi-level relative imports (`from .sub.pkg import x`) don't resolve correctly since `Module` nodes are keyed by bare filename with no package path.
...
- `devgraph/` — Phase 1 + Phase 2 + Phase 3 implementation:
  ...
  - `indexer/python/` — Tree-sitter-based Python extractor (`tree-sitter` + `tree-sitter-python`), per the Implementation Plan. Relative-import `IMPORTS` targets resolve to the sibling file's `Module` node name (`from . import x` -> `x.py`, `from .pkg import y` -> `pkg.py`); absolute/stdlib imports keep their bare name and are expected not to resolve to a same-repo node.
  ...
  - `indexer/dispatch.py` — routes changed/deleted files to the right extractor by name/extension (`index_paths`, `remove_paths`, `full_scan`). This is what `devgraph add`/`rescan`, the watcher's `on_changes` callback, and the tray app all now call — previously nothing did, so extractors existed but the graph never actually got populated automatically.
  ...
  - `indexer/git_history/` — Phase 3 `GitHistoryExtractor` (GitPython, purely local, no network) + `index_repo_history`. Incrementally walks commits newer than the registry's `last_indexed_commit`, creating `Commit` nodes and `MODIFIES` edges to `Module` nodes.
...
- `tests/` — mirrors `devgraph/` structure; 175 tests, run via `.venv/Scripts/python -m pytest`.
