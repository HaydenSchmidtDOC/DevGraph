...
def index_paths(engine: GraphEngine, repo_id: str, repo_root: Path, paths: set[Path], docs_path: str | None = None, mentions_enabled: bool = False) -> int:
    ...
    # kept as a parallel list rather than folded into the Python one: Java
    # has no datastore/API/service cross-linking pass, so reusing the same
    # list would either drag Java files through that Python-only logic or
    # require threading a language flag through it.
