...
IGNORED_DIR_NAMES = {".git", ".venv", "venv", "__pycache__", "build", "dist", ".pytest_cache", ".devgraph", "target"}
...
def index_paths(engine: GraphEngine, repo_id: str, repo_root: Path, paths: set[Path], docs_path: str | None = None, mentions_enabled: bool = False) -> int:
    ...
    # Parallel bookkeeping for Rust files -- same same-batch-ordering problem
    # (a CALLS/IMPORTS edge from file X to file Y in the same batch can miss
    # if X is processed before Y's Module node exists), same two-pass fix.
    # Kept as its own list rather than folded into py_files/py_extractions:
    # Rust has no datastore/API/service cross-linking pass (those extractors
    # are Python-source-specific), so reusing that machinery would mean
    # threading a "is this actually Python" check through it instead of
    # just not calling it.
    rs_files: list[str] = []
